 ...
 * muddy brown, which isn't what "vivid" should look like. The "dark" stop's
 * lightness floor also adapts to hue (see darkLightnessForHue) since yellow
 * goes muddy/brown at a lightness red and blue are still fine at. When a
 * genuine second hue was found in the image (e.g. a teal shirt against a
 * yellow wall), it gets its own stop in the gradient instead of the whole
 * hero being flattened to one color.
 */
export function heroGradientFrom(color) {
  ...
  return `linear-gradient(135deg, ${dark} 0%, ${mid} 40%, ${mid2} 60%, ${dark2} 100%)`;
}
